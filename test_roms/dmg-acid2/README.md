...
`docs/GAMEBOY_ROADMAP.md`'s licensing note), this one has an explicit
permissive license, so it's safe to keep here.

The standard PPU correctness test in the Game Boy dev community - see
`make gameboy-visual-test` (`tests/compare_frame.py`) to run it
...
as of Phase 3. See `docs/GAMEBOY_ROADMAP.md`'s Phase 3 status for the
